//
//  WHAddSubtractDateViewController.swift
//  DateCalculator
//
...
class WHAddSubtractDateViewController: WHDateDifferenceViewController {
    
    let addSubtractDateCellId = "addSubtractCellId"
    
    
    @objc var MKimage: UIImageView!
   override func animate(_ sender: UIButton) {
        let animation:CABasicAnimation = CABasicAnimation(keyPath: "position")
        animation.fromValue = NSValue(cgPoint:CGPoint(x: MKimage.frame.midX, y: MKimage.frame.midY))
        animation.toValue = NSValue(cgPoint:CGPoint(x: MKimage.frame.midX, y: 340))
        animation.duration = 1.0
        MKimage.layer.add(animation, forKey: "position")
    }
...
extension WHAddSubtractDateViewController: WHAddSubtractTableViewCellDelegate {
    func updateDataModel(dayMonthYear: [Int]) {
        self.dayMonthYear = dayMonthYear
    }
}

extension WHAddSubtractDateViewController {
